...
默认会保存会话 messages，但不会自动恢复旧历史；恢复需要显式传入 `-r <id>` / `--resume <id>`。如果不记得 id，可以先运行 `-s` / `--sessions`。不再需要的历史可用 `-x <id>` / `--delete <id>` 删除。如果需要完全关闭落盘，使用 `-t` / `--temp`。
...
高风险操作（例如编辑文件、创建文件或执行命令）会触发运行时审批，除非已启用 `-a` / `--auto`、`YOYO_SILENT=true` 或 `YOYO_AUTO_APPROVE=true`。
