...
2. ACP stdio MVP 已实现首版：`initialize`、`session/new`、`session/load`、`session/prompt`、`session/cancel`、`session/update`、`session/request_permission`。
...
外部 `yoyohub` 对接与兼容性反馈请按 [ACP 与 yoyohub 对接要求](acp_yoyohub_integration_requirements.md) 填写，重点提供环境信息、JSON-RPC 样例、session 生命周期测试、permission 测试、update 映射结果和问题复现材料。
...
| --- | --- | --- | --- |
| JSON-RPC transport | stdio newline-delimited JSON-RPC；stdout 只能输出 ACP 消息 | 已实现 `agent/acp/server.py`，stdout 只写 JSON-RPC，诊断走 stderr | 后续按真实 client 兼容测试打磨 |
| `initialize` | 协商 protocol version、capabilities、agentInfo、authMethods | 已返回 protocol/capabilities/agentInfo/authMethods | 需跟随 ACP schema 变化调整字段 |
| `session/new` | 使用 `cwd` 创建 session，返回 sessionId | 已实现 ACP handler，创建 workdir-bound `Session` | 后续补 MCP servers intake |
| `session/load` | 恢复 session 并 replay conversation | 已通过 `Session.replay_view()` 转 `session/update` replay | replay 粒度后续可增强 |
| `session/resume` | 无 replay 恢复会话 | 当前 CLI resume 会加载 messages | 可后续做；MVP 可不声明 |
| `session/prompt` | 接收 ContentBlock[]，运行一轮 agent，返回 stopReason | 已实现 ContentBlock -> prompt text，并复用 `Session.send()` | 后续需真实 client 兼容测试 |
| `session/cancel` | 取消当前 turn，返回 `cancelled` stop reason | 已实现 ACP session-level `CancellationController` | 后续补更细 tool cancelled update |
| `session/update` | agent message、tool call、tool update、plan、commands、mode updates | 已实现 `StreamEvent` -> `session/update` adapter | 后续补更严格 schema 类型 |
| permission request | agent 请求 client 授权工具调用 | 已实现 `AcpApprovalAdapter` 发 `session/request_permission` | 后续核对不同 client 的返回字段 |
...
| --- | --- | --- | --- | --- | --- |
| ACP server / 外部 agent 集成 | Zed 文档列出 Codex external agent；Zed 安装 `codex-acp` | Zed 文档列出 Claude Agent | Gemini CLI 是 Zed 初始参考 ACP 实现，配置含 `--acp` | 官方 public preview，`copilot --acp --stdio` / TCP | stdio server MVP 已实现，待 Zed/真实 client 兼容测试 |
...
P1.5 ACP stdio server MVP（已实现首版）
