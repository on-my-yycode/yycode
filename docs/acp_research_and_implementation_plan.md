...
1. 先实现 ACP stdio MVP：`initialize`、`session/new`、`session/load`、`session/prompt`、`session/cancel`、`session/update`、`session/request_permission`。
2. 工具仍由 yoyoagent 本地执行，不依赖 client fs/terminal 能力。
3. 把 `StreamEvent` 转换为 ACP `session/update`，把 `ApprovalRequest` 转换为 ACP permission request。
4. 后续再实现 session list、slash commands、session modes、client fs/terminal、MCP servers 转发和 registry 发布。
...
```text
P1 ACP compatibility prerequisites
